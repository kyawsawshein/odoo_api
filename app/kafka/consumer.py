...
class OdooMessageHandler:
    ...
    async def handle_product_message(self, message: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.error(
                "Failed to process product message",
                error=str(e),
                message=message
            )
    ...
    async def handle_inventory_message(self, message: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.error(
                "Failed to process inventory message",
                error=str(e),
                message=message
            )
    ...
    async def handle_bulk_sync_message(self, message: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.error(
                "Failed to process bulk sync message",
                error=str(e),
                message=message
            )
...
def create_odoo_consumer():
    ...
    consumer.register_handler("odoo-purchase", handler.handle_contact_message)  # Reuse for now
    consumer.register_handler("odoo-sales", handler.handle_contact_message)  # Reuse for now
