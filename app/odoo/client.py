...
class OdooClient:
    ...
    async def authenticate(self) -> Optional[int]:
        ...
        try:
            print("Odoo client ==================================: ", self.db, self.username, self.password)
    ...
    async def update_contact(self, contact_id: int, contact_data: Dict) -> bool:
        ...
        return await self.execute_kw('res.partner', 'write', [[contact_id], contact_data])
    
    async def search_contacts(self, domain: List = None, fields: List = None) -> List[Dict]:
        ...
        return await self.execute_kw('res.partner', 'search_read', [domain], {'fields': fields})
    ...
    async def update_product(self, product_id: int, product_data: Dict) -> bool:
        ...
        return await self.execute_kw('product.product', 'write', [[product_id], product_data])
    
    async def search_products(self, domain: List = None, fields: List = None) -> List[Dict]:
        ...
        return await self.execute_kw('product.product', 'search_read', [domain], {'fields': fields})
    ...
    async def get_stock_quantities(self, product_ids: List[int] = None) -> List[Dict]:
        ...
        return await self.execute_kw('stock.quant', 'search_read', [domain], {
    ...
    async def validate_delivery(self, delivery_id: int) -> bool:
        """Validate delivery"""
        return await self.execute_kw('stock.picking', 'button_validate', [[delivery_id]])
    ...
    # Generic Operations
    async def search_records(self, model: str, domain: List = None, fields: List = None, limit: int = None) -> List[Dict]:
    ...
    async def read_record(self, model: str, record_id: int, fields: List = None) -> Dict:
        """Read specific record"""
        if fields is None:
            fields = ['id', 'name']
        
        records = await self.execute_kw(model, 'read', [[record_id]], {'fields': fields})
