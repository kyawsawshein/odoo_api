...
class ProjectService(BaseService):
    ...
    async def get_user(sekf, odoo_client, user_ids: List[int]) -> List[User]:
    ...
    async def get_tag(sekf, odoo_client, user_ids: List[int]) -> List[ProjectTag]:
        tag_fields = list(ProjectTag.model_fields.keys())
        tags = await odoo_client.read_records(
            model="res.users",
            record_ids=user_ids,
            fields=tag_fields,
        )
