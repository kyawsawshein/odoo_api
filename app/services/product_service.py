...
class ProductService(BaseService):
    ...
    async def create_product(self, product_data: Dict[str, Any]) -> SyncResponse:
        ...
        try:
            ...
            from app.api.schemas import Product as ProductSchema
    ...
    async def update_product(self, product_id: int, update_data: Dict[str, Any]) -> SyncResponse:
    ...
    async def sync_products_from_odoo(self) -> SyncResponse:
        """Sync products from Odoo to local database"""
        try:
            odoo_client = await self._get_odoo_client()
            odoo_products = await odoo_client.search_products()
            ...
            for odoo_product in odoo_products:
                ...
                if existing_product:
                    ...
                    for field, value in odoo_product.items():
                    ...
                    product_data = {
                        ...
                        'type': odoo_product.get('type', 'product'),
                        'odoo_id': odoo_product['id'],
                        'user_id': self.current_user.id
                    }
                    db_product = ProductSchema(**product_data)
                    self.db.add(db_product)
