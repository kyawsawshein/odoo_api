...
class InventoryService(BaseService):
    ...
    async def sync_inventory(self) -> SyncResponse:
        ...
        try:
            ...
            for stock_item in odoo_stock:
                ...
                stmt = select(InventorySchema).where(
                    InventorySchema.product_id == product_id[0],  # Odoo returns [id, name]
                    InventorySchema.user_id == self.current_user.id
                )
                result = await self.db.execute(stmt)
                ...
                if existing_inventory:
                    ...
                    existing_inventory.location_id = stock_item.get('location_id')[0] if stock_item.get('location_id') else None
                else:
                    # Create new inventory record
                    inventory_data = {
                        'product_id': product_id[0],
                        'quantity': stock_item.get('quantity', 0),
                        'location_id': stock_item.get('location_id')[0] if stock_item.get('location_id') else None,
    ...
    async def update_inventory_level(self, product_id: int, quantity: float) -> SyncResponse:
        ...
        try:
            ...
            if inventory.odoo_id:
                ...
                adjustment_data = {
                    ...
                    'location_ids': [(6, 0, [inventory.location_id])] if inventory.location_id else [],
                    'state': 'confirm'
                }
